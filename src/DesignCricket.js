...
const DesignCricket = () => {
  ...
  const { score, isLoading } = useSelector((state) => ({
    score: state.cricketScore.score,
    isLoading: state.cricketScore.isLoading,
  }));
  const { id } = score;
  ...
  return (
    <>
      <div className="w-full   bg-red-600 py-2 px-2">
        <div className="text-white ml-2 font-semibold my-2 text-xl">
          CricApp
        </div>
      </div>
      {isLoading ? (
        // <div className="flex flex-row ">
        //   <div className="loader mt-5 ml-5"></div>
        //   <div className="mt-6 ml-2 text-bold text-indigo-400 ">
        //     Loading ...
        //   </div>
        // </div>
        <div className="loader flex flex-col">
          <div className="loader__element"></div>
          <div className="text-center pt-2 font-bold text-emerald-700">
            Loading...
          </div>
        </div>
      ) : (
